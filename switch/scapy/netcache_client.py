...
def main():
    pkt = (
        ...
        / IP()
        / netcache(psn=0, reuse_distance=0, is_req=0x1, op=0x0, key4=1, value4=0)
    )
    ...
    pkt = (
        Ether(dst="10:70:fd:2f:d8:51", src="10:70:fd:19:00:95")
        / IP()
        / netcache(psn=1, reuse_distance=0, is_req=0x1, op=0x0, key4=2, value4=0)
    )
    print("==== Request packet ====")
    hexdump(pkt)
    resp = srp1(pkt, iface=iface, timeout=1, verbose=False)
    ...
    pkt = (
        Ether(dst="10:70:fd:2f:d8:51", src="10:70:fd:19:00:95")
        / IP()
        / netcache(psn=2, reuse_distance=0, is_req=0x1, op=0x0, key4=3, value4=0)
    )
    print("==== Request packet ====")
    hexdump(pkt)
    resp = srp1(pkt, iface=iface, timeout=1, verbose=False)
